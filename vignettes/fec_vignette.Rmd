...
invisible(utils::capture.output(candidates_30 <- fec16::all_candidates(n_max = 30)))
```
...
```{r}
ggplot(wins %>% head(6), aes(x = reorder(cand_name, total_votes), y = total_votes, fill = cand_id)) +
  geom_col() +
...
```{r, eval=TRUE}
all_contributions <- fec16::all_contributions()
