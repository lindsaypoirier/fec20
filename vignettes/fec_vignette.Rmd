...
The `fec16` package provides tamed datasets of the Federal Election Commission (FEC)'s 2015-2016 election cycle. Some datasets are included in full, while a sample of the others is available with the option of retrieving the entire datasets through the functions built in the package. Details are given below.
...
Anyone interested in US politics and elections who wants to use actual data to think critically and make inferences. We made this package particularly with students and instructors in mind therefore this collection of data is good for teaching. 
...
invisible(utils::capture.output(all_candidates <- fec16::all_candidates()))
wins <- left_join(results_president, all_candidates) %>%
...
invisible(utils::capture.output(all_contributions <- fec16::all_contributions()))
results_by_cand <- left_join(results_house, all_candidates, by = "cand_id") %>% 
...
```{r message=FALSE, warning=FALSE}
house_winners <-left_join(results_house, all_candidates, by = "cand_id") %>%
