...
`house_results`: the results of the 2016 Congressional elections
...
The first six rows of the `house_results` dataset look like:
```{r}
head(house_results)
```
...
Using the `president_results` dataset, we can also see what actually happened in the 2016 elections.

Here is how we can summarize the number of wins by candidate:
```{r}
wins <- president_results %>%
